            ...
            try:
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        ...
                        if "GTA5_Enhanced.exe" in cmdline or "GTA5.exe" in cmdline:
                            console.print(
                                f"{proc.info['name']} detected.",
                                style="yellow"
                            )
                            return proc
                ...
                console.print(f"Detection error: {e}", style="red")
...
class WindowFocusManager:
    ...
    def __init__(self, process_name: str = GameDetector().get_gta_process().name()):
        ...
        if sys.platform == "win32":
            ...
            hook_type = ctypes.CFUNCTYPE

        self._hook_func = hook_type(
...
class FirewallManager:
    ...
    def rule_exists(self) -> bool:
        ...
        result = subprocess.run(
            f'netsh advfirewall firewall show rule name="{self.rule_name}"',
            shell=True, 
            capture_output=True, 
            text=True,
